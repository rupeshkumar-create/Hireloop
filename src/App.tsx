...
import { JobSearchPage } from './pages/RemoteJobsPage';
...
const JillDashboard = lazy(() => import('./pages/JillDashboard').then((m) => ({ default: m.JillDashboard })));
...
function PrivateRoute({ children, skipOnboarding }: { children: React.ReactNode; skipOnboarding?: boolean }) {
  ...
  if (!skipOnboarding && !onOnboarding && !onboardingDone) {
...
function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  const meta: Record<string, { eyebrow: string; title: string }> = {
    '/chat': { eyebrow: 'Your career agent', title: 'Jack.' },
    '/dashboard': { eyebrow: "Today's matches", title: 'Discover fresh roles.' },
    '/settings': { eyebrow: 'Account', title: 'Settings.' },
    '/jobs': { eyebrow: 'Pipeline', title: 'Track saved roles and introductions.' },
        ...
        <div className="hs-main">
          <header className="hs-topbar">
            ...
            <div className="hs-actions flex items-center gap-2">
              {!onChat ? (
                <Link to="/chat" className="hs-btn hs-btn-primary">
              ...
              {!onChat && !hideManualScout ? (
                <Link to="/chat" className="hs-btn">
                  Search jobs
    ...
    <AuthProvider>
      ...
      <Router>
        ...
        <Routes>
          ...
          <Route path="/remote-jobs" element={<Navigate to="/job-search" replace />} />
          ...
          <Route path="/chat" element={
            <PrivateRoute>
              <AppLayout>
                <LazyPage><ChatAssistant /></LazyPage>
              </AppLayout>
            </PrivateRoute>
          } />

          <Route path="/jill" element={
            <PrivateRoute skipOnboarding>
              <LazyPage><JillDashboard /></LazyPage>
            </PrivateRoute>
          } />
