...
describe('cronSchedule', () => {
  ...
  it('marks daily alerts due when tick fires at 08:00 UTC', () => {
  ...
  it('still runs daily alerts later the same UTC day (catch-up for late cron)', () => {
  ...
  it('skips daily alerts before the scheduled hour', () => {
    const schedule = CRON_JOBS.find((j) => j.id === 'daily-alerts')!;
  ...
  it('getDueCronJobs returns all jobs when forced', () => {
    expect(getDueCronJobs(new Date('2026-01-01T03:00:00.000Z'), { force: 'all' })).toHaveLength(1);
  });
});
