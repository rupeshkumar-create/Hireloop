/**
 * Seed 10 evergreen blog posts into Firestore with full Content Growth fields.
 * Idempotent — skips if already seeded unless force=true.
 */
import {
  loadStrategy,
  saveStrategy,
...
import {
  buildEvergreenMarkdown,
  extractFaqFromMarkdown,
} from './evergreen/buildArticle.js';
...
export interface SeedEvergreenResult {
  seeded: boolean;
  created: string[];
  skipped: string[];
...
export async function seedEvergreenPosts(options: { force?: boolean } = {}): Promise<SeedEvergreenResult> {
  const force = options.force ?? false;
  const state = await loadGrowthState();
  const existing = await listBlogPosts(100);
  const existingSlugs = new Set(existing.map((p) => p.slug));
  const evergreenSlugs = EVERGREEN_SPECS.map((s) => s.slug);

  const alreadySeeded =
    state.evergreenSeeded === true ||
    evergreenSlugs.filter((slug) => existingSlugs.has(slug)).length >= EVERGREEN_SPECS.length;

  if (alreadySeeded && !force) {
    return {
      seeded: false,
      created: [],
      skipped: evergreenSlugs,
      wordCounts: {},
      strategyPendingTopics: (await loadStrategy())?.pendingTopics.length ?? 0,
      message: 'Evergreen posts already seeded. Pass force=true to overwrite.',
    };
  }
  ...
  for (const spec of EVERGREEN_SPECS) {
    ...
    if (!meetsMinimumWordCount(content)) {
      throw new Error(`Evergreen seed "${spec.slug}" is below ${BLOG_TARGET_WORD_COUNT} words`);
    }
    ...
    const internalLinks = buildInternalLinks(
      ...
      EVERGREEN_SPECS.filter((s) => s.slug !== spec.slug).map((s) => ({
        slug: s.slug,
        title: s.title,
  ...
  // Bidirectional links mesh
  for (const post of builtPosts) {
    const related = post.internalLinks?.map((l) => l.slug) ?? [];
  ...
  const seededTitles = EVERGREEN_SPECS.map((s) => s.title);
  ...
  await saveGrowthState({
    ...
    totalPostsPublished: Math.max(state.totalPostsPublished ?? 0, EVERGREEN_SPECS.length),
    systemStatus: 'idle',
    lastError: null,
  });
  ...
  await logGrowthRun({
    type: 'evergreen_seed',
    status: 'success',
    details: { created, skipped, wordCounts, count: EVERGREEN_SPECS.length },
  });
  ...
  return {
    ...
    message: `Seeded ${created.length} evergreen posts (${BLOG_TARGET_WORD_COUNT}+ words each). Daily automation will add post #11 tomorrow.`,
  };
}
