/**
 * Seed 25 competitor alternative posts (SEO + GEO comparison guides).
 */
import {
  loadStrategy,
  saveStrategy,
...
export async function seedCompetitorPosts(options: { force?: boolean } = {}): Promise<SeedCompetitorsResult> {
  const force = options.force ?? false;
  const state = await loadGrowthState();
  const existing = await listBlogPosts(200);
  const existingSlugs = new Set(existing.map((p) => p.slug));
  const competitorSlugs = COMPETITOR_SPECS.map((s) => s.slug);

  const alreadySeeded =
    competitorSlugs.filter((slug) => existingSlugs.has(slug)).length >= COMPETITOR_SPECS.length;

  if (alreadySeeded && !force) {
    return {
      seeded: false,
      created: [],
      skipped: competitorSlugs,
      wordCounts: {},
      strategyPendingTopics: (await loadStrategy())?.pendingTopics.length ?? 0,
      message: 'Competitor posts already seeded. Pass force=true to overwrite.',
    };
  }

  let strategy = (await loadStrategy()) ?? (await initializeStrategy());
  const builtPosts: EnhancedBlogPost[] = [];
  const linkPool = COMPETITOR_SPECS.map((s) => ({
    slug: s.slug,
    title: s.title,
  ...
  for (const spec of COMPETITOR_SPECS) {
    ...
    if (!meetsMinimumWordCount(content)) {
      throw new Error(`Competitor seed "${spec.slug}" is below ${BLOG_TARGET_WORD_COUNT} words`);
    }
    ...
    const internalLinks = buildInternalLinks(
      ...
      linkPool.filter((s) => s.slug !== spec.slug)
    );
    ...
    const competitorName = spec.tags[0] ?? 'Competitor';
    const fixed = autoFixSeoFields({
      ...
      internalLinks,
      entityTags: ['HireSchema', competitorName, 'alternative', 'remote jobs', 'comparison', spec.clusterId],
    });
    ...
    await applyBidirectionalLinks(
      {
        ...
        clusterId: post.clusterId ?? 'competitor-alternatives',
      },
      related
    );
  }

  const seededTitles = COMPETITOR_SPECS.map((s) => s.title);
  ...
  await logGrowthRun({
    type: 'evergreen_seed',
    status: 'success',
    details: { type: 'competitor_alternatives', created, skipped, wordCounts, count: COMPETITOR_SPECS.length },
  });
  ...
  return {
    ...
    message: `Seeded ${created.length} competitor alternative posts (${BLOG_TARGET_WORD_COUNT}+ words each).`,
  };
}
