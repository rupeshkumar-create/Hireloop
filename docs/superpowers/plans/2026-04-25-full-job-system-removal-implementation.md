...
**Goal:** Remove the live job research and matching subsystem from the app while keeping the rest of the product stable and buildable.

**Architecture:** Replace the dashboard jobs experience with a frontend-safe placeholder, remove admin job simulation, then delete the runtime routes, services, types, tests, and docs that power live job generation and delivery. Keep only non-job product surfaces and shared AI features that do not require the deleted pipeline.
...
- `src/lib/jobLinks.ts`
- `JOB_ENGINE_FLOW.md`
- job-only design/plan docs that describe the live jobs pipeline as current behavior
...
- [ ] **Step 5: Commit the matches-tab simplification**

```bash
git add src/components/dashboard/MatchesTab.tsx src/components/dashboard/__tests__/MatchesTab.test.ts
git commit -m "refactor: replace matches tab with placeholder"
```
...
Replace `src/hooks/useDashboardJobs.ts` with this minimal version:
...
- [ ] **Step 4: Run typecheck to catch any remaining dashboard/job-type errors**
...
- [ ] **Step 5: Commit the dashboard de-coupling**

```bash
git add src/hooks/useDashboardJobs.ts src/pages/Dashboard.tsx src/types/dashboard.ts
git commit -m "refactor: remove live dashboard jobs integration"
```
...
- [ ] **Step 1: Remove ghost-mode tests and modal/types files that only exist for job simulation**
...
- [ ] **Step 2: Run typecheck to surface every remaining admin import break**
...
At the top of `src/pages/AdminDashboard.tsx`, remove these imports:
...
Replace any ghost-mode action button with nothing, or delete the containing action cell if it is job-only.

- [ ] **Step 4: Run typecheck again and verify admin page is clean**
...
- [ ] **Step 5: Commit the admin cleanup**

```bash
git add src/pages/AdminDashboard.tsx
git rm src/services/adminGhostMode.ts src/components/admin/GhostModeModal.tsx src/types/adminGhostMode.ts src/services/__tests__/adminGhostMode.test.ts
git commit -m "refactor: remove admin job simulation"
```

---

### Task 4: Delete Runtime Job Routes, Services, Types, And Email Digest Logic
...
Trim `src/services/emailService.ts` down to the shared `sendResendEmail()` helper and the signup email:
...
- [ ] **Step 4: Run a global search for live-job references and fix any remaining compile paths**
...
```text
Only historical comments or unrelated docs remain; no active runtime source files import or reference the deleted jobs pipeline.
```

- [ ] **Step 5: Run typecheck and build after the mass deletion**
...
- [ ] **Step 6: Commit the runtime deletion**

```bash
git add src/services/emailService.ts vercel.json
git rm api/jobs/index.ts api/cron/daily-alerts.ts api/cron/process-user.ts scripts/generate-daily-jobs.ts src/services/cronEngine.ts src/services/jobResearcher.ts src/services/jobMatchingEngine.ts src/types/dailyJob.ts src/services/__tests__/cronEngine.test.ts src/components/dashboard/JobDetailsPanel.tsx src/components/dashboard/matchPaywall.ts src/lib/jobLinks.ts
git commit -m "refactor: remove live job runtime pipeline"
```
...
- Delete: job-specific plans/specs that describe the active jobs engine

- [ ] **Step 1: Rewrite SYSTEM_FLOW to remove the live jobs pipeline**

Replace the current job-pipeline-specific sections in `SYSTEM_FLOW.md` with a short current-state summary like:
...
Do not delete the new removal design and implementation plan files created for this removal project.
...
```text
Remaining hits are either the new removal docs or comments explicitly marking the jobs system as removed.
```

- [ ] **Step 5: Commit the documentation cleanup**

```bash
git add SYSTEM_FLOW.md CRON_FLOW.md
git rm JOB_ENGINE_FLOW.md docs/superpowers/specs/2026-04-24-personalized-daily-job-delivery-design.md docs/superpowers/plans/2026-04-24-personalized-daily-job-delivery-implementation.md
git commit -m "docs: remove live job system documentation"
```
...
**Files:**
- Verify: `src/contexts/AuthContext.tsx`
- Verify: `src/services/aiService.ts`
- Verify: `src/services/learningSignals.ts`
- Verify: `firestore.rules`

- [ ] **Step 1: Verify that the remaining app still compiles without the deleted job types and services**
...
- [ ] **Step 2: Run the focused test suite that still matters after removal**
...
```text
vite build succeeds and emits dist output
```

- [ ] **Step 4: Run one final stale-reference search**
...
```bash
git diff --stat
git diff -- src/hooks/useDashboardJobs.ts src/pages/Dashboard.tsx src/pages/AdminDashboard.tsx src/components/dashboard/MatchesTab.tsx src/types/dashboard.ts src/services/emailService.ts vercel.json SYSTEM_FLOW.md CRON_FLOW.md
```

Expected:

```text
Diff shows the live jobs subsystem removed and placeholder/dashboard/admin/docs updates only
```
...
```bash
git add -A
git commit -m "refactor: remove live job system from app"
```
