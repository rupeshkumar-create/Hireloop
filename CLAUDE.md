# HireSchema — Claude Code Context
...
- **AI Gateway**: `openai` npm package pointed at `https://openrouter.ai/api/v1` — **not** `@anthropic-ai/sdk`, not direct OpenAI. Every model (Claude, Gemini, GPT) is called through OpenRouter using the same `openai.chat.completions.create()` interface.
...
| `email_generation`, `resume_tailoring`, `text_improvement` | `anthropic/claude-opus-4.6` |
...
→ Top-15 Enrichment (Claude: matchReasons, skillGaps, aiSummary, hotSignals)
